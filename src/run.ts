...
export type { RunTaskName, TaskSpec } from "./tasks"
...
export interface RunRequest {
  ...
  output?: string
  /** Reference name for the result package (default run-output:latest). */
  tag?: string
...
export type RequestOverlay = { [K in keyof RunRequest]?: RunRequest[K] | undefined }
...
export function addRunOptions(cmd: Command): Command {
  return cmd
    ...
    .option("--tag <repo:tag>", "Reference name for the result package (default run-output:latest)")
    ...
    .option("--plain-http", "Use HTTP for the registry (run packages)")
}
...
interface TaskFlagSpec {
  add: (cmd: Command, task: RunTaskName) => void
  when: (spec: TaskSpec, task: RunTaskName) => boolean
}
...
export function addTaskOptions(cmd: Command, task: RunTaskName): Command {
...
function addTaskArguments(cmd: Command, task: RunTaskName): void {
  const spec = TASKS[task]
  if (task === "resume") {
...
export function buildRunTaskCommand(task: RunTaskName): Command {
...
/**
 * The `run <ref>` handler re-parses trailing flags BEFORE the package's
  ...
  if (first !== undefined && !ctx.hasDefaultProvider) {
    throw new Error(
      `expected <provider>, got '${first}' (known providers: ${[...ctx.known].join(", ") || "none configured"}); ` +
        "or set defaults.run.provider via `creatifact config set defaults.run.provider <id>`",
    )
  }
...
function applyResumePayload(
  ...
  if (payload.length > 1) {
    throw new Error(
      `too many positional arguments for resume (${payload.slice(1).join(" ")}); the form is: run resume <handle|file>`,
    )
  }
...
function resolveFlagPrompt(o: RunCommandOptions): string | undefined {
...
function collectFlagFields(task: RunTaskName, o: RunCommandOptions, overlay: RequestOverlay): void {
...
function collectScalarFlagFields(o: RunCommandOptions, overlay: RequestOverlay): void {
...
export function parseRunArgs(
  ...
  const { options, positionals } = parseArgsWith<RunCommandOptions>(buildRunTaskCommand(task), args)
...
function validateRequiredImages(spec: TaskSpec, req: RunRequest): void {
...
function validateRequired(spec: TaskSpec, req: RunRequest): void {
...
function failForbiddenImages(req: RunRequest): void {
  if (req.task === "text2image") {
    fail("text2image does not take --image; image-to-image is `run image2image --image ...`")
  }
  if (req.task === "text2video") {
    fail("text2video does not take --image; image-to-video is `run image2video --image ...`")
  }
...
function validateForbiddenTiming(spec: TaskSpec, req: RunRequest): void {
...
function validateForbidden(spec: TaskSpec, req: RunRequest): void {
...
function failForbiddenFrames(req: RunRequest): void {
  const flag = hasValue(req.firstFrame) ? "--first-frame" : "--last-frame"
  if (req.task === "text2video") {
    fail(`text2video does not take ${flag}; use \`run image2video --image <img>\``)
  }
  if (req.task === "image2video") {
    fail(
      "image2video uses --image (it becomes the first frame); first+last frames belong to `run frames2video`",
    )
  }
  fail(`${req.task} does not accept ${flag}`)
}

function validateForbiddenPackaging(spec: TaskSpec, req: RunRequest): void {
...
export function validateRequest(req: RunRequest): void {
  const spec = TASKS[req.task]
  if (spec === undefined) fail(`unknown task '${req.task}'`)
 ...
 */
export function mergeRequest(base: RunRequest, overlay: RequestOverlay): RunRequest {
  const out: RunRequest = { ...base }
  for (const key of Object.keys(overlay) as Array<keyof RunRequest>) {
    const value = overlay[key]
    if (value === undefined) continue
    if (key === "options") {
      out.options = { ...(base.options ?? {}), ...(overlay.options ?? {}) }
    } else if (key === "task") {
      out.task = value as RunTaskName
    } else {
 ...
 */
export function pickModelForTask(provider: Provider, task: RunTaskName): PickedModel {
  const spec = TASKS[task]
...
async function suggestModelsForTask(
  ...
  try {
    ...
    return [
      ...
      ...lines,
      `run \`creatifact run ${task} --list-models\` for more`,
    ].join("\n")
  } catch {
...
function resolveTargetIds(
  ...
  const providerId = defaultRunProvider(loadConfig(opts.configPath)) ?? ""
  if (providerId === "") {
    fail(
      "no <provider> given and no default provider configured; " +
        "set defaults.run.provider via `creatifact config set defaults.run.provider <id>`, " +
...
async function runTextTask(ctx: ExecCtx): Promise<RunResult> {
...
async function runUnderstandTask(ctx: ExecCtx): Promise<RunResult> {
  const { req, provider, model, signal } = ctx
...
async function runEmbedTask(ctx: ExecCtx): Promise<RunResult> {
  const { req, provider, model, signal } = ctx
...
async function executeTask(
  ...
  RunResult | { artifacts: Artifact[]; usage: Usage | undefined } | { handle: JobHandle }
> {
  switch (ctx.req.task) {
    case "text2text":
...
export function effectiveRunSpec(req: RunRequest, providerId: string, model: string): RunSpec {
  const spec: RunSpec = { task: req.task, provider: providerId, model }
...
/** Everything a run command can produce; the envelope's `data`. */
...
async function packageTextResultIfRequested(
  ...
  const resultTag = req.tag ?? "run-output:latest"
  ...
  const pkg = await buildResultPackage({
    ...
    artifacts: [],
    spec: effectiveRunSpec(req, provider.id, model),
...
function inputRefTarget(req: RunRequest, ref: InputProvenance): string | undefined {
  switch (ref.field) {
    case "images":
...
function applyReplacements(runReq: RunRequest, replacements: Map<string, string>): RunRequest {
  const apply = (v: string): string => replacements.get(v) ?? v
  const out: RunRequest = { ...runReq }
 ...
 * (anchored by run.inputRefs digests). Returns undefined when no input is
...
async function buildInputFallback(
  ...
  runReq: RunRequest,
  configPath: string | undefined,
): Promise<{ req: RunRequest; cleanup: () => void; replaced: number } | undefined> {
 ...
 * with stored bytes per run.inputRefs. Anything else surfaces as-is.
 */
  ...
  try {
    ...
    warn(
      `provider rejected an input url; retrying with stored bytes ` +
        `(${fallback.replaced} input${fallback.replaced === 1 ? "" : "s"}, per run.inputRefs)`,
    )
    try {
...
function looksLikeRunRef(arg: string, storeTags?: Set<string>): boolean {
...
async function materializePackageMedia(
  req: RunRequest,
  image: LoadedImage,
): Promise<{ req: RunRequest; cleanup: () => void }> {
  const usesPkg =
...
function rejectPkgRefsOutsidePackageMode(req: RunRequest): void {
  const fields = [
  ...
  if (fields.some((v) => v.startsWith("pkg://"))) {
    fail("`pkg://` references only work with `creatifact run <ref>` (a run package)")
  }
}
...
const TASK_DESCRIPTIONS: Record<RunTaskName, string> = {
...
/**
 * The `run` command tree: one subcommand per task plus a parent handler
 * for the `run <ref>` package mode and unknown tasks.
 */
export function buildRunCommand(): Command {
  const run = new Command("run")
    .usage("<task>")
    .description(
      "Run a generation task (text2image, image2video, frames2video, ...) or a run package ref",
    )
  // Options after the first operand belong to the task subcommand (or to the
  // `run <ref>` handler, which re-parses them); only --config-dir is
  // consumed here.
  run.enablePositionalOptions()
  addGlobalOptions(run)
  run.allowExcessArguments(true)
  ...
  run.allowUnknownOption()

  for (const task of Object.keys(TASKS) as RunTaskName[]) {
    const cmd = run.command(task).description(TASK_DESCRIPTIONS[task])
    addTaskOptions(cmd, task)
    addTaskArguments(cmd, task)
    ...
    cmd.action(async (...invocation: unknown[]) => {
      ...
      const options = (invocation.at(-2) ?? {}) as RunCommandOptions
      ...
      if (options.listModels === true) {
        ...
        emitResult("run", result, prettyOpts(command))
        return
      }
      const overlay = overlayFromParsed(task, args, options, providerContext(opts), {})
      const result = await executeRunRequest(mergeRequest({ task }, overlay), opts)
      emitResult("run", result, prettyOpts(command))
    })
  }

  run.action(async (options: { configDir?: string }, command: Command) => {
    const args = command.args
    if (args.length === 0) {
      command.help()
      return
    ...
    if (looksLikeRunRef(head, storeTags)) {
      // The trailing operands include the task flags; parse them here.
      const { options: taskOpts, positionals } = parseArgsWith<RunCommandOptions>(
        buildRefTaskCommand(),
        args.slice(1),
      )
      const result = await runPackageRef(
        head,
        positionals,
        taskOpts,
    ...
    fail(`unknown task '${head}' (expected ${TASK_LIST}, or a run package ref)`)
  })
